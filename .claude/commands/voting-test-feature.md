---
description: Writes and runs tests for a specific voting feature. Pass the spec name as argument e.g. /test-feature 05-auth
...
"Please provide a spec name. Usage: /test-feature <spec-name> e.g. /test-feature 05-auth"
...
- Source files to read for structure:
  - `app.py`
  - `routers/` directory, if present
  - `models/` or `schemas/` directory, if present
  - `database/` directory, if present
- Output test file to create:
  `tests/test_$ARGUMENTS.py`
- Instruction: Write tests based on what the spec says the feature should do. Do NOT derive test logic from reading the implementation. Cover happy paths, edge cases, auth guards, validation errors, and DB side effects where applicable.

Wait for test-writer to fully complete and confirm the test file has been written before proceeding to Step 2.

## Step 2: Run Tests

Once test-writer has finished, invoke the **test-runner** subagent with the following context:
...
- If test-writer reports it could not write the test file, stop and report the reason — do NOT proceed to Step 2.
