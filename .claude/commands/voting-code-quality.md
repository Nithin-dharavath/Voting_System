---
description: Runs parallel security and quality code review for a specific voting feature. Pass the spec name as argument e.g. /code-review-feature 03-login
...
### voting-security-reviewer receives:
- The combined diff from the pre-flight check.
- Spec file for context: `.claude/specs/$ARGUMENTS.md`
- Source files to reference: `app.py` and `database/` directory, if present.
...
- The combined diff from the pre-flight check.
- Spec file for context: `.claude/specs/$ARGUMENTS.md`
- Source files to reference: `app.py`, `database/` directory, and `templates/` directory, if present.
- Instruction: Review only the changed code for quality, framework best practices, and maintainability. Do not comment on security concerns.
