...
def test_apply_page_active_election():
    ...
    token = get_student_token()
    response = client.get("/student/elections/2/apply", cookies={COOKIE_NAME: token}, follow_redirects=False)
...
def test_apply_page_ended_election():
    ...
    token = get_student_token()
    response = client.get("/student/elections/3/apply", cookies={COOKIE_NAME: token}, follow_redirects=False)
    assert response.status_code == 302
    assert response.headers["location"] == "/student/elections"
...
def test_apply_page_already_applied():
    ...
    token = get_student_token()
    with get_db_cursor() as cursor:
        cursor.execute("INSERT INTO candidate_applications (user_id, election_id, manifesto, approval_status) VALUES (1, 1, 'My Manifesto', 'PENDING')")

    response = client.get("/student/elections/1/apply", cookies={COOKIE_NAME: token}, follow_redirects=False)
    assert response.status_code == 302
...
def test_submit_application_success():
    ...
    with get_db_cursor() as cursor:
        cursor.execute("SELECT id, approval_status FROM candidate_applications WHERE user_id = 1 AND election_id = 1")
        row = cursor.fetchone()
...
def test_submit_application_already_applied():
    ...
    with get_db_cursor() as cursor:
        cursor.execute("INSERT INTO candidate_applications (user_id, election_id, manifesto, approval_status) VALUES (1, 1, 'Existing', 'PENDING')")
...
def test_view_candidate_status_success():
    ...
    token = get_student_token()
    with get_db_cursor() as cursor:
        cursor.execute("INSERT INTO candidate_applications (user_id, election_id, manifesto, approval_status) VALUES (1, 1, 'Manifesto 1', 'PENDING')")
        cursor.execute("INSERT INTO candidate_applications (user_id, election_id, manifesto, approval_status) VALUES (1, 2, 'Manifesto 2', 'APPROVED')")
