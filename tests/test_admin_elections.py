...
def test_create_election_post_student(student_client):
    response = student_client.post(
        "/admin/elections", data={"title": "Test"}, follow_redirects=False
    )
    assert response.status_code == 302
    assert response.headers["location"] == "/admin/login"
...
def test_update_election_post_unauthenticated(client):
    response = client.post(
        "/admin/elections/1/update", data={"title": "Test"}, follow_redirects=False
    )
    assert response.status_code == 302
    assert response.headers["location"] == "/admin/login"
...
def test_create_election_success(admin_client, mock_cursor):
    ...
    assert (
        args[0]
        == "INSERT INTO elections (title, description, start_time, end_time, status, created_by) VALUES (%s, %s, %s, %s, %s, %s)"
    )
...
def test_create_election_bad_format(admin_client, mock_cursor):
    ...
    assert response.status_code == 200
    assert "Invalid date/time format" in response.text
    mock_cursor.execute.assert_not_called()
...
def test_edit_election_page_admin(admin_client, mock_cursor):
    ...
    mock_cursor.execute.assert_called_with(
        "SELECT id, title, description, start_time, end_time, result_published, status, created_by FROM elections WHERE id = %s",
        (1,),
    )
...
def test_update_election_success(admin_client, mock_cursor):
    ...
    assert (
        args[0]
        == "UPDATE elections SET title = %s, description = %s, start_time = %s, end_time = %s, status = %s WHERE id = %s"
    )
...
def test_delete_election_success(admin_client, mock_cursor):
    ...
    response = admin_client.post(
        "/admin/elections/1/delete", data={"csrf_token": csrf_token}, follow_redirects=False
    )
